...
test('scenario service selects and launches public scenarios', () => {
  ...
  expect(scenarios.current).toMatchObject({ id: 'act-01-level-1', source: 'campaign', origin: 'start' });
...
test('scenario service enforces developer visibility and supports restart', () => {
  ...
  expect(scenarios.select('legacy-movement-lab')).toMatchObject({ ok: false, reason: 'developer-only' });
  expect(scenarios.launch('legacy-movement-lab')).toMatchObject({ ok: false, reason: 'developer-only' });

  game.session.developerModeOverride = true;
  expect(scenarios.launch('legacy-movement-lab', { origin: 'url' })).toMatchObject({ ok: true });
  expect(scenarios.restartCurrent()).toMatchObject({ ok: true, scenario: { id: 'legacy-movement-lab' } });
  expect(game.scenarioCalls).toEqual([
    ['switchLevel', 'legacy-movement-lab'],
    ['switchLevel', 'legacy-movement-lab']
  ]);
});
