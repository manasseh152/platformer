...
test('editor viewport allows padded panning around maps that are smaller than the visible camera', () => {
  const view = viewport({ width: 800, height: 600, zoom: 1, x: 0, y: 0 });

  clampCamera(view, 128, 96);

  expect(view.camera.x).toBe(-116);
  expect(view.camera.y).toBe(-32);
});
...
test('editor viewport allows padded panning past map borders when the map is larger than the camera', () => {
  const view = viewport({ width: 320, height: 180, zoom: 1, x: 900, y: 900 });

  clampCamera(view, 1000, 600);

  expect(view.camera.x).toBe(900);
  expect(view.camera.y).toBe(640);
});
