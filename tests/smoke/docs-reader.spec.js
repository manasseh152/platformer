...
test('Docs reader renders one selected markdown file with sidebar navigation', async ({ page }) => {
  ...
  const firstPath = await page.locator('.doc-card--header p').last().textContent();
  ...
  await expect(articles).toHaveCount(1);
  await expect(page.locator('.doc-card--header')).toContainText(secondPath.trim());
  await expect(page.locator('.doc-card--header')).not.toContainText(firstPath.trim());
