...
function buildTerrainNeighborMask(scene, col, row) { let mask = 0; const bit = (dx, dy, value) => isSolidTileAt(scene, col + dx, row + dy) ? value : 0; mask |= bit(0, -1, 1); mask |= bit(1, -1, 2); mask |= bit(1, 0, 4); mask |= bit(1, 1, 8); mask |= bit(0, 1, 16); mask |= bit(-1, 1, 32); mask |= bit(-1, 0, 64); mask |= bit(-1, -1, 128); return mask; }
