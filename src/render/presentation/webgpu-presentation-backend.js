import { getPreferredCanvasFormat } from '../../gpu/device.js';
...
export async function createWebGpuPresentationBackend(canvas, gpuContext) {
  ...
  return {
    kind: 'webgpu-presentation-backend',
    mode: 'webgpu',
    canvas,
    device,
    context,
    present(source, viewport) {
      ...
      device.queue.copyExternalImageToTexture({ source: source.canvas }, { texture: sourceTexture }, [source.width, source.height]);
