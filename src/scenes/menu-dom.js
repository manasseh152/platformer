  ...
  return `<div id="startScreen" class="ds-screen ds-screen--hero scene-start-screen">
    <main class="start-stage menu-card ds-panel ds-panel--hero" aria-labelledby="startTitle">
      <section class="hero-copy" aria-label="Game introduction">
        ...
        <div class="menu-actions ds-action-stack"><button id="startButton" class="ds-button ds-button--primary">${renderIconLabel('play', 'Start')}</button><button id="startLevelSelectButton" class="secondary ds-button ds-button--secondary">${renderIconLabel('list', 'Level Select')}</button><button id="startEditorButton" class="secondary ds-button ds-button--secondary">${renderIconLabel('map', 'Map Editor')}</button><button id="startSettingsButton" class="secondary ds-button ds-button--secondary">${renderIconLabel('settings', 'Settings')}</button></div>
      </section>
      <aside class="hero-scene" aria-label="Quest preview"><div class="moon" aria-hidden="true"></div><div class="hero-scene--badge">Act 01</div><div class="hero-scene--title">Hollow Gate</div><div class="hero-pixel hero-pixel--torch" aria-hidden="true"></div><div class="hero-runner" aria-hidden="true"><div class="runner-head"></div><div class="runner-body"></div><div class="runner-sword"></div></div><div class="hero-gate" aria-hidden="true"></div><div class="hero-ground" aria-hidden="true"></div><dl class="hero-loadout" aria-label="Run loadout"><div><dt>Objective</dt><dd>Reach gate</dd></div><div><dt>Threat</dt><dd>Spike floor</dd></div><div><dt>Skill</dt><dd>Dash ready</dd></div></dl></aside>
    </main>
  </div>`;
}
...
function pauseOverlayMarkup() {
  return `<div id="pauseScreen" class="ds-screen ds-screen--overlay scene-pause-overlay" data-menu-page="main" data-menu-direction="forward">
    <div class="menu-card ds-panel"><div class="eyebrow ds-overline" id="menuEyebrow">Paused</div><h2 id="menuTitle" class="ds-title">Paused</h2><div id="menuPages" class="menu-pages">
      <div id="pauseMainPage" class="pause-main menu-page" data-page="main"><p>Take a break, restart the run, or change settings.</p><div class="menu-actions ds-action-stack"><button id="resumeButton" class="ds-button ds-button--primary">${renderIconLabel('play', 'Continue')}</button><button id="levelSelectButton" class="secondary ds-button ds-button--secondary">${renderIconLabel('list', 'Level Select')}</button><button id="settingsButton" class="secondary ds-button ds-button--secondary">${renderIconLabel('settings', 'Settings')}</button><button id="restartButton" class="secondary ds-button ds-button--secondary">${renderIconLabel('rotateCcw', 'Restart')}</button><button id="mainMenuButton" class="secondary ds-button ds-button--secondary">${renderIconLabel('home', 'Main Menu')}</button><button id="closeGameButton" class="secondary ds-button ds-button--secondary" hidden>${renderIconLabel('x', 'Close to Editor')}</button></div></div>
