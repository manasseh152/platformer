...
function fallbackSummary(game) {
  ...
  const issues = pipeline.deferredFallbackReason ?? pipeline.webglFallbackReason ?? [];
...
export function registerRenderPipelineDevTools(game) {
  game.devTools.registry.registerSection({
    ...
    items: [
      ...
      { id: 'backend-selected', kind: 'value', label: 'Backend selected', get: game => pipelineDiagnostics(game)?.selectedBackendKind ?? 'Not rendered yet' },
